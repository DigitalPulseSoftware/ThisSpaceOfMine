...
return {
    PlanetType = "torus_planet",
    Generator = function (chunk, chunkcount, parameters)
        local radius = assert(parameters.Radius, "missing Radius property")
        local thickness = assert(parameters.Thickness, "missing Thickness property")
        --perlin:reseed(seed)
        ...
        for z = 0, chunksize - 1 do
            for y = 0, chunksize - 1 do
                ...
                --[[local depth = maxHeight.x - math.max(0, sdTorus(blockPos, Vec2(20, 50)))
                    ...
                    local dist = sdTorus(blockPos, Vec2(radius, thickness))
                    if dist < 0 then
