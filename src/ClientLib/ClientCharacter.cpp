...
namespace tsom
{
	ClientCharacter::ClientCharacter(Nz::EnttWorld& world, const Nz::Vector3f& position, const Nz::Quaternionf& rotation)
	{
		...
		m_controller = std::make_shared<ClientCharacterController>();
	...
	void ClientCharacter::SetLastInputs(const PlayerInputs& lastInputs)
	{
		m_controller->SetInputs(lastInputs);
	}
